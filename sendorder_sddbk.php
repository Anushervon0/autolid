...
include 'recaptcha-verification.php';
...
$formData = [
   ...
   //Новый осаго
   'vehicle-registration-number' => $_POST['vehicle-registration-number'] ?? null,
   'vehicle-vin-number' => $_POST['vehicle-vin-number'] ?? null,
   'preferred-contact-method' => $_POST['preferred-contact-method'] ?? null,
   'contact-user-id' => $_POST['contact-user-id'] ?? null,

   'owner-first-name' => $_POST['owner-first-name'] ?? null,
   'owner-last-name' => $_POST['owner-last-name'] ?? null,
   'owner-middle-name' => $_POST['owner-middle-name'] ?? null,
   'owner-birth-date' => $_POST['owner-birth-date'] ?? null,
   
   'driver-first-name-1' => $_POST['driver-first-name-1'] ?? null,
   'driver-first-name-2' => $_POST['driver-first-name-2'] ?? null,
   'driver-first-name-3' => $_POST['driver-first-name-3'] ?? null,
   'driver-first-name-4' => $_POST['driver-first-name-4'] ?? null,
   'driver-first-name-5' => $_POST['driver-first-name-5'] ?? null,

   'driver-last-name-1' => $_POST['driver-last-name-1'] ?? null,
   'driver-last-name-2' => $_POST['driver-last-name-2'] ?? null,
   'driver-last-name-3' => $_POST['driver-last-name-3'] ?? null,
   'driver-last-name-4' => $_POST['driver-last-name-4'] ?? null,
   'driver-last-name-5' => $_POST['driver-last-name-5'] ?? null,

   'driver-middle-name-1' => $_POST['driver-middle-name-1'] ?? null,
   'driver-middle-name-2' => $_POST['driver-middle-name-2'] ?? null,
   'driver-middle-name-3' => $_POST['driver-middle-name-3'] ?? null,
   'driver-middle-name-4' => $_POST['driver-middle-name-4'] ?? null,
   'driver-middle-name-5' => $_POST['driver-middle-name-5'] ?? null,

   'driver-birth-date-1' => $_POST['driver-birth-date-1'] ?? null,
   'driver-birth-date-2' => $_POST['driver-birth-date-2'] ?? null,
   'driver-birth-date-3' => $_POST['driver-birth-date-3'] ?? null,
   'driver-birth-date-4' => $_POST['driver-birth-date-4'] ?? null,
   'driver-birth-date-5' => $_POST['driver-birth-date-5'] ?? null,
];
...
/**
 * Обрабатывает выбранный способ связи и подготавливает данные для Битрикс24
 *
 * @param array $formData Данные из $_POST
 * @return array Массив с готовыми значениями для полей Лида
 */
function prepareContactFields(array $formData): array 
{
    // Внутренняя карта ID способов связи Битрикс24
    $bitrixMapping = [
        'CALL'     => 1070, // Звонок
        'TELEGRAM' => 1071, // Telegram
        'MAX'      => 5407, // MAX
        'WHATSAPP' => 1072, // WhatsApp
        'EMAIL'    => 1820, // E-Mail
    ];

    $method = $formData['preferred-contact-method'] ?? 'CALL';
    $mainPhone = $formData['phone'] ?? '';
    
    // Если текстовое поле пустое — берем основной телефон
    $inputValue = !empty($formData['contact-user-id']) ? $formData['contact-user-id'] : $mainPhone;

    // Инициализируем поля мессенджеров пустыми значениями
    $tgValue = '';
    $waValue = '';
    $maxValue = '';

    // Заполняем только то поле, метод которого выбрал пользователь
    match($method) {
        'TELEGRAM' => $tgValue  = $inputValue,
        'WHATSAPP' => $waValue  = $inputValue,
        'MAX'      => $maxValue = $inputValue,
        default    => null
    };

    // Возвращаем готовый набор полей для Битрикса
    return [
        "UF_CRM_1744807660434"       => isset($bitrixMapping[$method]) ? [$bitrixMapping[$method]] : [],
        "UF_CRM_TELEGRAMUSERNAME_WZ" => $tgValue,
        "UF_CRM_WHATSAPPLID_WZ"      => $waValue,
        "UF_CRM_MAXID_WZ"            => $maxValue,
    ];
}
...
function sendMail($formData, $formType)
{
   ...
   $recaptchaAction = $_POST['recaptcha_action'] ?? 'submit';
   
   if (empty($recaptchaToken)) {
      header('Content-Type: application/json');
      echo json_encode([
         'success' => false,
         'error' => 'Токен reCAPTCHA отсутствует'
      ]);
      exit;
   }
   
   $recaptchaResult = verifyRecaptcha($recaptchaToken, $recaptchaAction);
   ...
   error_log("reCAPTCHA проверка успешна.  Score: " . $recaptchaResult['score']);
   ...
   $domains = [
    ...
    $site_name = isset($domains[$subdomain]) ? (string) $domains[$subdomain]['name'] : '';
      ...
      // Водители ОСАГО-Новый
      $newOsagoDrivers = [];

      for ($i = 1; $i <= 5; $i++) {
         // Получаем все поля для текущего водителя
         $firstName  = $formData["driver-first-name-$i"] ?? null;
         $lastName   = $formData["driver-last-name-$i"] ?? null;
         $middleName = $formData["driver-middle-name-$i"] ?? null;
         $birthDate  = $formData["driver-birth-date-$i"] ?? null;
         $experience = $formData["driver-experience-$i"] ?? null;

         // Проверяем, заполнено ли хотя бы одно ключевое поле (например, имя или фамилия)
         if ($firstName || $lastName) {
            // Объединяем данные через двоеточие (или любой другой разделитель)
            $osagoDriver = implode(':', [
                  $lastName ?? '', 
                  $firstName ?? '', 
                  $middleName ?? '', 
    ...
    $config = [
        ...
        'landing_source' => "UC_CISI3I" //70
    ];

    $contactFields = prepareContactFields($formData);
      ...
      switch ($formType) {
         ...
         case 'osago-new-form':
            ...
            $bitrixParams = [
               "fields" => array_merge([
                  ...
                  "UF_CRM_1744807379192" => $formData['vehicle-vin-number'] ?? '', //VIN
                  "UF_CRM_1744632396" => $formData['vehicle-registration-number'] ?? '', // Гос. номер
                  "UF_CRM_1744646586" => $newOsagoDrivers, // Водители

               ], $contactFields)
            ];

            if($formData['forma-sobstvennosti'] == 'Юридическое лицо') {
               $bitrixParams["fields"]["UF_CRM_1751628607"] = 1868;
               $bitrixParams["fields"]["UF_CRM_1748342715974"] = $formData['inn'] ?? '';
               $bitrixParams["fields"]["UF_CRM_1748342798407"] = $formData['city'] ?? '';
            } else {
               $bitrixParams["fields"]["UF_CRM_1751628607"] = 1869;
               $bitrixParams["fields"]["UF_CRM_1781183276"] = ( $formData['owner-last-name'] ?? '' ) ." ". ( $formData['owner-first-name'] ?? '' ) ." ".
                  ( $formData['owner-middle-name'] ?? '' ) .", Дата рождения:". ( $formData['owner-birth-date'] ?? '' ); //Собственник
               $bitrixParams["fields"]["UF_CRM_1771395881625"] = $formData['city'] ?? '';
            }

            // print_r($bitrixParams);
         ...
         case 'go-form':
            ...
            $bitrixParams = [
               "fields" => array_merge([
                  ...
                  "UF_CRM_1775121524909" =>	(int) preg_replace('/\D/', '', $formData['object-price'] ?? 0 ),
                  "UF_CRM_1775121557402" => $formData['go-object-type'] ?? '',
         ...
         case 'property-yurlic-form':
            ...
            $bitrixParams = [
               "fields" => array_merge([
                  ...
                  "UF_CRM_1771395881625" => $formData['city'] ?? '',
                  "UF_CRM_1748342715974" => $formData['inn'] ?? '',
                  "UF_CRM_1744632437" =>	(int) preg_replace('/\D/', '', $formData['object-price'] ?? 0 ),
         ...
         case 'dms-nsl-form':
            ...
            $bitrixParams = [
               "fields" => array_merge([
                  ...
                  "UF_CRM_1750408424570" => (int) preg_replace('/\D/', '', $formData['insurance-amount'] ?? 0),
         ...
         case 'kv-form':
            ...
            foreach($formData['insurance-objects'] as $key => $val){
               ...
               if( $amountTypeKey ) {
                  $insuranceAmount = (int) preg_replace('/\D/', '', $formData['obj-insurance-amount'][$amountTypeKey] ?? 0);
                  $isSet = "Да";
         ...
         case 'gruz-form':
            ...
            $bitrixParams = [
               "fields" => array_merge([
                  ...
                  "UF_CRM_1748342798407" => $formData['city'] ?? '',
                  "UF_CRM_1748342715974" => $formData['inn'] ?? '',
                  "UF_CRM_1750407407102" => (int) preg_replace('/\D/', '', $formData['total-cargo-price'] ?? 0),
         ...
         case 'opo-form':
            ...
            $bitrixParams = [
               "fields" => array_merge([
                  ...
                  "UF_CRM_1771396744775" => $formData['dangerous-object-location'] ?? '',
                  "UF_CRM_1775596594" => $formData['dangerous-object'],
      ...
      $fieldLabels = [
         ...
         //Новый осаго
         'vehicle-registration-number' => 'Гос. номер автомобиля',
         'vehicle-vin-number' => 'Vin. номер автомобиля',
         'preferred-contact-method' => 'Предпочитаемый способ связи',
         'contact-user-id' => 'Никнейм в предпочитаемом спомобе связи',

         'owner-first-name' => 'Имя владельца ТС',
         'owner-last-name' => 'Фамилия владельца ТС',
         'owner-middle-name' => 'Отчество владельца ТС',
         'owner-birth-date' => 'Дата рождения владельца ТС',
         
         'driver-first-name-1' => 'Имя 1-го водителя',
         'driver-first-name-2' => 'Имя 2-го водителя',
         'driver-first-name-3' => 'Имя 3-го водителя',
         'driver-first-name-4' => 'Имя 4-го водителя',
         'driver-first-name-5' => 'Имя 5-го водителя',

         'driver-last-name-1' => 'Фамилия 1-го водителя',
         'driver-last-name-2' => 'Фамилия 2-го водителя',
         'driver-last-name-3' => 'Фамилия 3-го водителя',
         'driver-last-name-4' => 'Фамилия 4-го водителя',
         'driver-last-name-5' => 'Фамилия 5-го водителя',

         'driver-middle-name-1' => 'Отчество 1-го водителя',
         'driver-middle-name-2' => 'Отчество 2-го водителя',
         'driver-middle-name-3' => 'Отчество 3-го водителя',
         'driver-middle-name-4' => 'Отчество 4-го водителя',
         'driver-middle-name-5' => 'Отчество 5-го водителя',

         'driver-birth-date-1' => 'Дата рождения 1-го водителя',
         'driver-birth-date-2' => 'Дата рождения 2-го водителя',
         'driver-birth-date-3' => 'Дата рождения 3-го водителя',
         'driver-birth-date-4' => 'Дата рождения 4-го водителя',
         'driver-birth-date-5' => 'Дата рождения 5-го водителя',
      ...
      // Полис
      if (!empty($_FILES['insurance-policy-img']['name'][0])) { // Проверяем первый элемент массива
      ...
      // ПТС
      if (!empty($_FILES['insurance-policy-pts']['name'][0])) { 
         foreach ($_FILES['insurance-policy-pts']['tmp_name'] as $index => $tmpName) {
            if ($_FILES['insurance-policy-pts']['error'][$index] === UPLOAD_ERR_OK) {   
               $ptsFiles[] = [
                  "fileData" => [    
                     $_FILES['insurance-policy-pts']['name'][$index],
                     base64_encode(file_get_contents($tmpName))
                  ]
               ];  
            }
      ...
      if (!empty($policyFiles) || !empty($ptsFiles)) {

         if (!empty($policyFiles)) {
            $bitrixParams['fields']['UF_CRM_1745414837964'] = $policyFiles;
         }

         if (!empty($ptsFiles)) {
            $bitrixParams['fields']['UF_CRM_1781183248'] = $ptsFiles;
         }
      }

      $res = CRest::call('crm.lead.add', $bitrixParams);
      ...
      $files = [];
        ...
        if (!empty($_FILES['insurance-policy-img']['name'])) {
            foreach ($_FILES['insurance-policy-img']['tmp_name'] as $index => $tmpName) {
                if ($_FILES['insurance-policy-img']['error'][$index] === UPLOAD_ERR_OK) {
                    $files["file[$index]"] = curl_file_create(
                        $tmpName,
                        $_FILES['insurance-policy-img']['type'][$index],
                        $_FILES['insurance-policy-img']['name'][$index]
                    );
                }
            }
        ...
        // Убедимся, что все значения в $param - строки
        ...
        // Условно объединяем массивы
        $allParams = array_merge(
            $param,
            !empty($files) ? $files : [],
            [
                'authorization_login' => 'clients',
                'authorization_pass' => 'foiuth3i4yeh4inu44vc9w4',
            ]
        );
        ...
        if (!empty($res)) {
         if (isset($res['error'])) {
            $errorMsg = isset($res['error_description']) ? $res['error_description'] : $res['error'];
            error_log("Ошибка API Битрикс24: " . $errorMsg);
            
            echo json_encode([
                  'success' => false,
                  'error' => 'Ошибка создания лида',
                  'apiError' => $res['error'],
                  'apiDescription' => $errorMsg
            ], JSON_UNESCAPED_UNICODE);
            
         } else if (isset($res['result'])) {
            error_log("API отправка успешна. ID лида: " . $res['result']);
            
            echo json_encode([
                  'success' => true,
                  'lead_id' => $res['result'], // Содержит ID созданного лида (например, 36339)
                  'apiResponse' => $res,
                  'fileUploaded' => isset($tmpName) ? $tmpName : null,
            ], JSON_UNESCAPED_UNICODE);
            
         } else {
            error_log("Ошибка: Неизвестный формат ответа от API. Ответ: " . print_r($res, true));
            echo json_encode([
                  'success' => false,
         ...
         error_log("Ошибка отправки в API: cURL или HTTP ошибка. Ответ пустой.");
         echo json_encode([
            'success' => false,
            'error' => 'Ошибка отправки данных в API',
         ], JSON_UNESCAPED_UNICODE);
      }
        
      //   $ch = curl_init();
      //   curl_setopt($ch, CURLOPT_URL, 'http://attiks.ru/admin/ajax/editClient.php?module=mod_zayavki&submodule=' . $module);
      //   curl_setopt($ch, CURLOPT_HEADER, false);
      //   curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      //   curl_setopt($ch, CURLOPT_POST, true);
      //   curl_setopt($ch, CURLOPT_POSTFIELDS, $allParams);
      //   curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);
      //   curl_setopt($ch, CURLOPT_USERAGENT, 'Web-Edition');
      ...
      //   if ($res) {
      //       $apiResponse = json_decode($res, true);
      //       if (json_last_error() === JSON_ERROR_NONE) {
      //           error_log("API отправка успешна: $res");
      //           echo json_encode([
      //               'success' => true,
      ...
      //           ]);
      //       } else {
      //           error_log("Ошибка: Некорректный ответ от API. Ответ: $res");
      //           echo json_encode([
      //               'success' => false,
      //               'error' => 'Некорректный ответ от API',
      //               'rawResponse' => $res,
      ...
      //       }
      //   } else {
      //       error_log("Ошибка отправки в API: $curlError (HTTP CODE: $httpCode)");
      //       echo json_encode([
      //           'success' => false,
      //           'error' => 'Ошибка отправки данных в API',
      //           'curlError' => $curlError,
